...
make_phy_and_data <- function(phy, data, out_string){
    ...
    if (length(attributes(phy)$dropped$dropped_from_data) >=1){
        out_string <- c(out_string, "\n## Some information might have been removed during analysis. \n \nBe aware that the function dropped some tips of your phylogeny:\n \n")
        table1 <- as.character( kable(attributes(phy)$dropped$dropped_from_data) )
    ...
    if (length(attributes(phy)$dropped$dropped_from_tree) >=1){
        table2 <- as.character( kable(attributes(phy)$dropped$dropped_from_tree) )
